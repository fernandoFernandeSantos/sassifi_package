...
__global__ void qsort_warp(unsigned *indata, unsigned *outdata,
        unsigned int offset, unsigned int len, qsortAtomicData *atomicData,
        qsortRingbuf *atomicDataStack, unsigned int source_is_indata,
        unsigned int depth) {
    ...
    if (lane_id == 0) {
        ...
        if (gt_count > 0)
            gt_offset = len
                    - (atomicAdd((unsigned int *) &atomicData->gt_offset,
                            gt_count) + gt_count);
    }
            ...
            if (lt_len > BITONICSORT_LEN) {
                ...
                if (depth >= QSORT_MAXDEPTH) {
                    ...
                    big_bitonicsort<<<1, BITONICSORT_LEN, 0, lstream>>>(outdata,
                            source_is_indata ? indata : outdata, indata, offset,
                            lt_len);
                } else {
                    // Launch another quicksort. We need to allocate more storage for the atomic data.
                    if ((atomicData = ringbufAlloc<qsortAtomicData>(
                            atomicDataStack)) == NULL)
                        printf(
                                "Stack-allocation error. Failing left child launch.\n");
                    else {
                        ...
                        qsort_warp<<<numblocks, QSORT_BLOCKSIZE, 0, lstream>>>(
                                outdata, indata, offset, lt_len, atomicData,
            ...
            if (gt_len > BITONICSORT_LEN) {
                ...
                if (depth >= QSORT_MAXDEPTH)
                    big_bitonicsort<<<1, BITONICSORT_LEN, 0, rstream>>>(outdata,
                            source_is_indata ? indata : outdata, indata,
                    ...
                    if ((atomicData = ringbufAlloc<qsortAtomicData>(
                            atomicDataStack)) == NULL)
                        printf(
                                "Stack allocation error! Failing right-side launch.\n");
                    else {
                        atomicData->lt_offset = atomicData->gt_offset =
                                atomicData->sorted_count = 0;
                        ...
                        qsort_warp<<<numblocks, QSORT_BLOCKSIZE, 0, rstream>>>(
                                outdata, indata, offset + lt_len, gt_len,
                                atomicData, atomicDataStack, !source_is_indata,
                                depth + 1);
                    }
                }
            } else if (gt_len > 1) {
...
double run_quicksort_cdp(parameters_t *params, cudaStream_t stream) {
    ...
    if (params->size > BITONICSORT_LEN) {
        unsigned int numblocks = (unsigned int) (params->size
                + (QSORT_BLOCKSIZE - 1)) >> QSORT_BLOCKSIZE_SHIFT;
        qsort_warp<<<numblocks, QSORT_BLOCKSIZE, 0, stream>>>(params->gpudata,
                params->scratchdata, 0U, params->size, gpustack, ringbuf, true,
                0);
    } else {
        bitonicsort<<<1, BITONICSORT_LEN>>>(params->gpudata, params->gpudata, 0,
                params->size);
    }
...
int dataRead(parameters_t *params) {
    ...
    if (finput = fopen(params->inputName, "rb")) { // READ INPUT
        ...
        if (!(finput = fopen(params->inputName, "wb"))) {
            printf(
                    "Warning! Couldn't write the input to file, proceeding anyway...\n");
        } else {
...
int checkKeys(parameters_t *params) { // Magicas que a semana anterior ao teste proporcionam
    ...
    range = (
            (numValues * sizeof(unsigned char) > 2048000000) ?
                    2048000000 : numValues); // Avoid more than 1GB of RAM alloc
...
void getParams(int argc, char *argv[], parameters_t *params) {
    ...
    if (checkCmdLineFlag(argc, (const char **) argv, "gold")) {
        ...
        snprintf(params->goldName, 100, "quickSortGold%i",
    ...
    if (checkCmdLineFlag(argc, (const char **) argv, "input")) {
        ...
        snprintf(params->inputName, 100, "quickSortInput%i",
