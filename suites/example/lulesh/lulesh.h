...
enum {
	VolumeError = -1, QStopError = -2, LFileError = -3
};
...
public:
	...
	void sortRegions(Vector_h<Int_t>& regReps_h,
	...
	Real_t& get_delv_xi(Index_t idx) {
		return d_delv_xi[idx];
	}
	Real_t& get_delv_eta(Index_t idx) {
		return d_delv_eta[idx];
	}
	Real_t& get_delv_zeta(Index_t idx) {
		return d_delv_zeta[idx];
	}

	Real_t& get_x(Index_t idx) {
		return d_x[idx];
	}
	Real_t& get_y(Index_t idx) {
		return d_y[idx];
	}
	Real_t& get_z(Index_t idx) {
		return d_z[idx];
	}

	Real_t& get_xd(Index_t idx) {
		return d_xd[idx];
	}
	Real_t& get_yd(Index_t idx) {
		return d_yd[idx];
	}
	Real_t& get_zd(Index_t idx) {
		return d_zd[idx];
	}

	Real_t& get_fx(Index_t idx) {
		return d_fx[idx];
	}
	Real_t& get_fy(Index_t idx) {
		return d_fy[idx];
	}
	Real_t& get_fz(Index_t idx) {
		return d_fz[idx];
	}

	// host access
	Real_t& get_nodalMass(Index_t idx) {
		return h_nodalMass[idx];
	}
	...
	void BuildMesh(Int_t nx, Int_t edgeNodes, Int_t edgeElems, Int_t domNodes,
			Int_t padded_domElems, Vector_h<Real_t> &x_h, Vector_h<Real_t> &y_h,
			Vector_h<Real_t> &z_h, Vector_h<Int_t> &nodelist_h);
