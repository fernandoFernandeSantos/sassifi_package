...
void print_usage(const char *exec_name) {
	printf(
			"Usage: %s -matrix_size=N <-reps=N> <-generate> <-generategold> <-input=inputpath> <-gold=goldpath> (optional)\n",
			exec_name);
	printf(
...
void hostSetup(parameters_t *params) {
	...
	if (params->generate == 1) { // Generate input and write to file
		...
		if (!(finput = fopen(params->inputName, "rb"))) {
			printf("Error: Could not open input file in rb mode. %s\n",
					params->inputName);
#ifdef LOGS
			log_error_detail("Error: Could not open input file in wb mode."); end_log_file();
#else
			exit(EXIT_FAILURE);
#endif
...
////////////////////// FAULT INJECTION
	...
	if (params->generate == 0) { // No generate, read gold from file.
		if (!(fgold = fopen(params->goldName, "rb"))) {
			printf("Error: Could not open gold file in rb mode. %s\n",
					params->goldName);
#ifdef LOGS
			log_error_detail("Error: Could not open gold file in rb mode. "); end_log_file();
#else
			exit(EXIT_FAILURE);
#endif
		} else {
...
void getParams(parameters_t *params, int argc, char *argv[]) {
	...
	if (checkCmdLineFlag(argc, (const char **) argv, "matrix_size")) {
		params->matrix_size = getCmdLineArgumentInt(argc, (const char **) argv,
				"matrix_size");
		...
		if (params->matrix_size <= 0) {
			printf("Invalid matrix size given on the command-line: %d\n",
					params->matrix_size);
#ifdef LOGS
			log_error_detail("Invalid matrix size given on the command-line"); end_log_file();
#else
			exit(EXIT_FAILURE);
#endif
		}
	...
	if (checkCmdLineFlag(argc, (const char **) argv, "reps")) {
		params->reps = getCmdLineArgumentInt(argc, (const char **) argv,
				"reps");
		...
		if (params->reps <= 0) {
			printf("Invalid reps given on the command-line: %d\n",
					params->reps);
#ifdef LOGS
			log_error_detail("Invalid reps given on the command-line"); end_log_file();
#else
			exit(EXIT_FAILURE);
#endif
		}
	...
	if (checkCmdLineFlag(argc, (const char **) argv, "generate")) { // 0 = no / 1 = input+gold / 2 = gold
		params->generate = 1;
	...
	if (checkCmdLineFlag(argc, (const char **) argv, "gold")) {
		...
		snprintf(params->goldName, 100, "lud_gold_%i",
				(signed int) (params->matrix_size));
	...
	if (checkCmdLineFlag(argc, (const char **) argv, "input")) {
		...
		snprintf(params->inputName, 100, "lud_input_%i",
				(signed int) (params->matrix_size));
...
int validateOutput(parameters_t *params) {
	...
	if (params->generate == 1) { // Write gold to file.
		FILE *fgold;
		...
		if (!(fgold = fopen(params->goldName, "wb"))) {
			printf("Error: Could not open gold file in wb mode. %s\n",
					params->goldName);
#ifdef LOGS
			log_error_detail("Error: Could not open gold file in wb mode."); end_log_file();
#else
			exit(EXIT_FAILURE);
#endif
		} else {
		...
		int m_dim = (int) sqrt(params->matrix_size);

#pragma omp parallel for
		for (int i = 0; i < m_dim; i++) {
			for (int j = 0; j < m_dim; j++) {
				if (ptr[i + m_dim * j] != goldptr[i + m_dim * j]) {
					printf("p: [%d, %d], r: %1.16e, e: %1.16e\n", i, j,
							ptr[i + m_dim * j], goldptr[i + m_dim * j]);

#pragma omp critical
					{
						host_errors++;
#ifdef LOGS
						snprintf(error_detail, 150, "p: [%d, %d], r: %1.16e, e: %1.16e", i,
								j, ptr[i + m_dim * j], goldptr[i + m_dim * j]);
...
int main(int argc, char *argv[]) {
...
#ifdef LOGS
	char test_info[90];
	snprintf(test_info, 90, "matrix_size:%d reps:%d", params.matrix_size, params.reps);
	start_log_file("cudaLud", test_info);
#endif
	for (int loop = 0; loop < params.reps; loop++) {
		...
		devSetup(&params);
		if (params.verbose)
			printf("Iteration[%d]: Setup time: %.3fs\n", loop,
					mysecond() - timer);
...
#endif
		if (params.verbose)
			printf("Iteration[%d]: Device time: %.3fs\n", loop,
					mysecond() - timer);

		timer = mysecond();
		errors = validateOutput(&params);
		if (params.verbose)
			printf("Iteration[%d]: Validation time %.3fs\n", loop,
					mysecond() - timer);
		...
		devRelease(&params);
		if (params.verbose)
			printf("Iteration[%d]: Global time %.3fs\n", loop,
