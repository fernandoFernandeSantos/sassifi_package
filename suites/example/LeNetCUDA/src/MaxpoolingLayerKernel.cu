...
//		output_[index] = max_in_cuda(input_, max_loc, in_width, in_height, out,
//				h_, w_, index);
...
//#pragma unroll
			for (size_t y = 0; y < MAXPOOL_SIZE; y++) {
				...
				if (max_pixel < input_[tmp]) {
					max_pixel = input_[tmp];
//					max_loc[index] = get_max_loc_pair(index, tmp);
					max_loc[index].first = index;
					max_loc[index].second = tmp;
				}
			}
		}
...
void call_backpropagation_maxpool(Pair *max_loc, float *g_, float *g_next,
		size_t max_size) {
	dim3 blocks, threads;
