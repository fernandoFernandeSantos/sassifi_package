...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus,
		int ngpus, int clear) {
	int i;
...
void validate_classifier_crop(char *datacfg, char *filename, char *weightfile) {
	...
	for (i = 1; i <= splits; ++i) {
		...
		printf("%d: top 1: %f, top %d: %f, %lf seconds, %d images\n", i,
				avg_acc / i, topk, avg_topk / i, sec(clock() - time),
				val.X.rows);
		free_data(val);
	}
}
...
void try_classifier(char *datacfg, char *cfgfile, char *weightfile,
		char *filename, int layer_num) {
...
void label_classifier(char *datacfg, char *filename, char *weightfile) {
	...
	for (i = 0; i < m; ++i) {
		...
		image crop = crop_image(resized, (resized.w - net->w) / 2,
				(resized.h - net->h) / 2, net->w, net->h);
		float *pred = network_predict(net, crop.data);
...
void test_classifier(char *datacfg, char *cfgfile, char *weightfile,
		int target_layer) {
	int curr = 0;
	network *net = load_network(cfgfile, weightfile, 0);
	srand(time(0));
...
void file_output_classifier(char *datacfg, char *filename, char *weightfile,
		char *listfile) {
	int i, j;
	network *net = load_network(filename, weightfile, 0);
	set_batch_network(net, 1);
	...
	for (i = 0; i < m; ++i) {
		...
		float *pred = network_predict(net, crop.data);
		if (net->hierarchy)
			hierarchy_predictions(pred, net->outputs, net->hierarchy, 0, 1);
...
void threat_classifier(char *datacfg, char *cfgfile, char *weightfile,
...
void gun_classifier(char *datacfg, char *cfgfile, char *weightfile,
		int cam_index, const char *filename) {
#ifdef OPENCV
...
void demo_classifier(char *datacfg, char *cfgfile, char *weightfile,
		int cam_index, const char *filename) {
#ifdef OPENCV
	char *base = basecfg(cfgfile);
...
void run_classifier(int argc, char **argv) {
	...
	if (0 == strcmp(argv[2], "predict"))
		...
		file_output_classifier(data, cfg, weights, filename);
