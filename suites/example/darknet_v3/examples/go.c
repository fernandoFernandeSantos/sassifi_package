...
void train_go(char *cfgfile, char *weightfile, char *filename, int *gpus,
	...
	while (get_current_batch(net) < net->max_batches || net->max_batches == 0) {
		...
		printf("%ld, %.3f: %f, %f avg, %f rate, %lf seconds, %ld images\n",
				get_current_batch(net), (float)(*net->seen) / N, loss,
		...
		if (get_current_batch(net) % 10000 == 0) {
			char buff[256];
			sprintf(buff, "%s/%s_%ld.backup", backup_directory, base,
...
static void propagate_liberty(float *board, int *lib, int *visited, int row,
		int col, int side) {
	if (row < 0 || row > 18 || col < 0 || col > 18)
		return;
	int index = row * 19 + col;
	if (occupied(board, index) != side)
		return;
	if (visited[index])
		return;
	visited[index] = 1;
	...
	for (j = 0; j < 19; ++j) {
		for (i = 0; i < 19; ++i) {
			...
			if (!occupied(board, index)) {
				if ((i > 0) && occupied(board, index - 1))
					propagate_liberty(board, lib, visited, j, i - 1,
							occupied(board, index - 1));
				if ((i < 18) && occupied(board, index + 1))
					propagate_liberty(board, lib, visited, j, i + 1,
							occupied(board, index + 1));
				if ((j > 0) && occupied(board, index - 19))
					propagate_liberty(board, lib, visited, j - 1, i,
...
void print_board(FILE *stream, float *board, int player, int *indexes) {
	...
	for (j = 0; j < 19; ++j) {
		...
		for (i = 0; i < 19; ++i) {
			...
			if (occupied(board, index) == player)
				fprintf(stream, " X");
			else if (occupied(board, index) == -player)
				fprintf(stream, " O");
			else
				fprintf(stream, " .");
		}
...
int compare_board(float *a, float *b) {
	if (memcmp(a, b, 19 * 19 * 3 * sizeof(float)) == 0)
		return 1;
	return 0;
}
...
float select_mcts(mcts_tree *root, network *net, float *prev, float cpuct) {
	...
	for (i = 0; i < 19 * 19 + 1; ++i) {
		root->prob[i] = root->mean[i]
				+ cpuct * root->prior[i] * sqrt(root->total_count)
						/ (1. + root->visit_count[i]);
		if (root->prob[i] > max) {
			max = root->prob[i];
			max_i = i;
		}
	...
	if (root->children[i]) {
		...
		if (max_i < 19 * 19
				&& !legal_go(root->board, prev, 1, max_i / 19, max_i % 19)) {
			root->mean[i] = -1;
			root->value[i] = -1;
			root->prior[i] = 0;
			--root->total_count;
...
mcts_tree *run_mcts(mcts_tree *tree, network *net, float *board, float *ko,
		int player, int n, float cpuct, float secs) {
	int i;
	double t = what_time_is_it_now();
	if (player < 0)
		flip_board(board);
	if (!tree)
		tree = expand(copy_board(board), ko, net);
	assert(compare_board(tree->board, board));
	for (i = 0; i < n; ++i) {
		if (secs > 0 && (what_time_is_it_now() - t) > secs)
			break;
		int max_i = max_int_index(tree->visit_count, 19 * 19 + 1);
		if (tree->visit_count[max_i] >= n)
			break;
...
move pick_move(mcts_tree *tree, float temp, int player) {
	...
	fprintf(stderr,
			...
			index / 19, index % 19, tree->result, tree->prior[index],
			probs[index], tree->mean[index],
			(tree->children[index]) ? tree->children[index]->result : 0,
			tree->visit_count[index]);
	int ind = max_index(probs, 19 * 19 + 1);
	fprintf(stderr,
			"%d %d, Result: %f, Prior: %f, Prob: %f, Mean Value: %f, Child Result: %f, Visited: %d\n",
			ind / 19, ind % 19, tree->result, tree->prior[ind], probs[ind],
			tree->mean[ind],
			(tree->children[ind]) ? tree->children[ind]->result : 0,
			tree->visit_count[ind]);
...
int check_ko(float *x, float *ko) {
	...
	if (curr[19 * 19 * 2] != ko[19 * 19 * 2])
		flip_board(curr);
	if (compare_board(curr, ko))
		return 1;
	return 0;
}
...
int print_game(float *board, FILE *fp) {
	...
	for (j = 0; j < 19; ++j) {
		for (i = 0; i < 19; ++i) {
			if (occupied(board, j * 19 + i) == 1)
				fprintf(fp, "play black %c%d\n", 'A' + i + (i >= 8), 19 - j);
			if (occupied(board, j * 19 + i) == -1)
				fprintf(fp, "play white %c%d\n", 'A' + i + (i >= 8), 19 - j);
			if (occupied(board, j * 19 + i))
				++count;
		}
	}
	return count;
...
mcts_tree *ponder(mcts_tree *tree, network *net, float *b, float *ko,
		int player, float cpuct) {
	double t = what_time_is_it_now();
	int count = 0;
	if (tree)
...
void engine_go(char *filename, char *weightfile, int mcts_iters, float secs,
		float temp, float cpuct, int anon, int resign) {
	mcts_tree *root = 0;
	...
	while (1) {
		...
		if (!strcmp(buff, "protocol_version")) {
			...
			int known = (!strcmp(comm, "protocol_version")
					|| !strcmp(comm, "name") || !strcmp(comm, "version")
					|| !strcmp(comm, "known_command")
					|| !strcmp(comm, "list_commands") || !strcmp(comm, "quit")
					|| !strcmp(comm, "boardsize")
					|| !strcmp(comm, "clear_board") || !strcmp(comm, "komi")
					|| !strcmp(comm, "final_status_list")
					|| !strcmp(comm, "play") || !strcmp(comm, "genmove_white")
			...
			printf("\n");
		} else if (!strcmp(buff, "play") || !strcmp(buff, "black")
				|| !strcmp(buff, "white")) {
			++move_num;
			char color[256];
			if (!strcmp(buff, "play")) {
		...
		} else if (!strcmp(buff, "genmove") || !strcmp(buff, "genmove_black")
				|| !strcmp(buff, "genmove_white")) {
			++move_num;
			int player = 0;
			...
			if (player > 0) {
				if (black_time_left <= 30)
					secs = 2.5;
				else
					secs = orig_time;
			} else {
				if (white_time_left <= 30)
					secs = 2.5;
				else
					secs = orig_time;
			}
			...
			double t = what_time_is_it_now();
			root = run_mcts(root, net, board, two, player, mcts_iters, cpuct,
					secs);
			fprintf(stderr, "%f Seconds\n", what_time_is_it_now() - t);
...
void run_go(int argc, char **argv) {
	...
	else if (0 == strcmp(argv[2], "engine"))
