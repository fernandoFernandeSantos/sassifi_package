...
void print_yolo_detections(FILE **fps, char *id, int total, int classes, int w,
		int h, detection *dets) {
	int i, j;
	for (i = 0; i < total; ++i) {
...
void validate_yolo(char *cfg, char *weights) {
	...
	for (i = nthreads; i < m + nthreads; i += nthreads) {
		...
		for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
			...
			print_yolo_detections(fps, id, l.side * l.side * l.n, classes, w, h,
					dets);
...
void run_yolo(int argc, char **argv) {
	...
	if (0 == strcmp(argv[2], "test"))
		...
		demo(cfg, weights, thresh, cam_index, filename, voc_names, 20,
