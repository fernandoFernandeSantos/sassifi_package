...
void optimize_picture(network *net, image orig, int max_layer, float scale,
		float rate, float thresh, int norm) {
	//scale_image(orig, 2);
	...
	image im = resize_image(crop, (int) (orig.w * scale),
			(int) (orig.h * scale));
	if (flip)
		flip_image(im);
			...
			for (i = 0; i < recon.w; ++i) {
				...
				for (jj = j - num; jj <= j + num && jj < recon.h; ++jj) {
					...
					for (ii = i - num; ii <= i + num && ii < recon.w; ++ii) {
						...
						update.data[out_index] +=
								lambda
										* (recon.data[in_index]
												- recon.data[out_index]);
					}
				}
			}
		}
...
void reconstruct_picture(network *net, float *features, image recon,
		image update, float rate, float momentum, float lambda,
...
void run_nightmare(int argc, char **argv) {
	...
	for (e = 0; e < rounds; ++e) {
		...
		for (n = 0; n < iters; ++n) {
			...
			fflush(stderr);
			if (reconstruct) {
				reconstruct_picture(net, features, im, update, rate, momentum,
						lambda, smooth_size, 1);
				//if ((n+1)%30 == 0) rate *= .5;
		...
		char buff[256];
		if (prefix) {
			sprintf(buff, "%s/%s_%s_%d_%06d", prefix, imbase, cfgbase,
					max_layer, e);
		} else {
			sprintf(buff, "%s_%s_%d_%06d", imbase, cfgbase, max_layer, e);
		}
		...
		image crop = crop_image(im, im.w * (1. - zoom) / 2.,
				im.h * (1. - zoom) / 2., im.w * zoom, im.h * zoom);
		image resized = resize_image(crop, im.w, im.h);
		free_image(im);
		free_image(crop);
		im = resized;
