...
long numops(network *net) {
	...
	for (i = 0; i < net->n; ++i) {
		...
		if (l.type == CONVOLUTIONAL) {
			ops += 2l * l.n * l.size * l.size * l.c / l.groups * l.out_h
					* l.out_w;
...
void normalize_net(char *cfgfile, char *weightfile, char *outfile) {
	...
	for (i = 0; i < net->n; ++i) {
		...
		if (l.type == GRU && l.batch_normalize) {
			*l.input_z_layer = normalize_layer(*l.input_z_layer,
					l.input_z_layer->outputs);
			*l.input_r_layer = normalize_layer(*l.input_r_layer,
					l.input_r_layer->outputs);
			*l.input_h_layer = normalize_layer(*l.input_h_layer,
					l.input_h_layer->outputs);
			*l.state_z_layer = normalize_layer(*l.state_z_layer,
					l.state_z_layer->outputs);
			*l.state_r_layer = normalize_layer(*l.state_r_layer,
					l.state_r_layer->outputs);
			*l.state_h_layer = normalize_layer(*l.state_h_layer,
					l.state_h_layer->outputs);
...
void denormalize_net(char *cfgfile, char *weightfile, char *outfile) {
	...
	for (i = 0; i < net->n; ++i) {
		layer l = net->layers[i];
		if ((l.type == DECONVOLUTIONAL || l.type == CONVOLUTIONAL)
				&& l.batch_normalize) {
			denormalize_convolutional_layer(l);
