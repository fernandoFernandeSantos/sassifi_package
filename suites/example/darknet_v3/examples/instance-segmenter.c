...
void train_isegmenter(char *datacfg, char *cfgfile, char *weightfile, int *gpus,
	...
	while (get_current_batch(net) < net->max_batches || net->max_batches == 0) {
		...
		if (display) {
			...
			image im = float_to_image(net->w, net->h, net->c,
					train.X.vals[net->batch * (net->subdivisions - 1)]);
			pred.c = 80;
...
void predict_isegmenter(char *datafile, char *cfg, char *weights,
		char *filename) {
...
void demo_isegmenter(char *datacfg, char *cfg, char *weights, int cam_index,
		const char *filename) {
#ifdef OPENCV
...
void run_isegmenter(int argc, char **argv) {
	...
	if (0 == strcmp(argv[2], "test"))
		predict_isegmenter(data, cfg, weights, filename);
	else if (0 == strcmp(argv[2], "train"))
		train_isegmenter(data, cfg, weights, gpus, ngpus, clear, display);
	else if (0 == strcmp(argv[2], "demo"))
		demo_isegmenter(data, cfg, weights, cam_index, filename);
}
