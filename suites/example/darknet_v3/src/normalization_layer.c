...
void resize_normalization_layer(layer *layer, int w, int h) {
	...
	layer->squared = realloc(layer->squared,
			h * w * c * batch * sizeof(float));
...
void forward_normalization_layer_gpu(const layer layer, network net)
{
	...
	int h = layer.h;
	int c = layer.c;
	scal_gpu(w*h*c*layer.batch, 0, layer.squared_gpu, 1, net.st);

	for(b = 0; b < layer.batch; ++b) {
		float *squared = layer.squared_gpu + w*h*c*b;
		float *norms = layer.norms_gpu + w*h*c*b;
		float *input = net.input_gpu + w*h*c*b;
		pow_gpu(w*h*c, 2, input, 1, squared, 1, net.st);

		const_gpu(w*h, layer.kappa, norms, 1, net.st);
		for(k = 0; k < layer.size/2; ++k) {
			axpy_gpu(w*h, layer.alpha, squared + w*h*k, 1, norms, 1, net.st);
		}

		for(k = 1; k < layer.c; ++k) {
			copy_gpu(w*h, norms + w*h*(k-1), 1, norms + w*h*k, 1, net.st);
			int prev = k - ((layer.size-1)/2) - 1;
			int next = k + (layer.size/2);
			if(prev >= 0) axpy_gpu(w*h, -layer.alpha, squared + w*h*prev, 1, norms + w*h*k, 1, net.st);
			if(next < layer.c) axpy_gpu(w*h, layer.alpha, squared + w*h*next, 1, norms + w*h*k, 1, net.st);
		}
	}
	pow_gpu(w*h*c*layer.batch, -layer.beta, layer.norms_gpu, 1, layer.output_gpu, 1, net.st);
	mul_gpu(w*h*c*layer.batch, net.input_gpu, 1, layer.output_gpu, 1, net.st);
}
...
void backward_normalization_layer_gpu(const layer layer, network net)
{
	...
	int h = layer.h;
	int c = layer.c;
	pow_gpu(w*h*c*layer.batch, -layer.beta, layer.norms_gpu, 1, net.delta_gpu, 1, net.st);
	mul_gpu(w*h*c*layer.batch, layer.delta_gpu, 1, net.delta_gpu, 1, net.st);
}
#endif
