...
__global__ void backward_scale_kernel(float *x_norm, float *delta, int batch,
...
void backward_scale_gpu(float *x_norm, float *delta, int batch, int n,
        int size, float *scale_updates, cudaStream_t st) {
...
__global__ void backward_bias_conn_kernel(float *bias_updates, float *delta,
        int batch, int n) {
    int index = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x
            + threadIdx.x;
    if (index >= n)
...
__global__ void backward_bias_kernel(float *bias_updates, float *delta,
...
__global__ void adam_kernel(int N, float *x, float *m, float *v, float B1,
...
extern "C" void adam_gpu(int n, float *x, float *m, float *v, float B1,
        float B2, float rate, float eps, int t, cudaStream_t st) {
    adam_kernel<<<cuda_gridsize(n), BLOCK, 0, st>>>(n, x, m, v, B1, B2, rate,
            eps, t);
    check_error(cudaPeekAtLastError());
}

extern "C" void adam_update_gpu(float *w, float *d, float *m, float *v,
        float B1, float B2, float eps, float decay, float rate, int n,
...
__global__ void normalize_kernel(int N, float *x, float *mean,
...
__global__ void normalize_delta_kernel(int N, float *x, float *mean,
        float *variance, float *mean_delta, float *variance_delta, int batch,
...
extern "C" void normalize_delta_gpu(float *x, float *mean, float *variance,
        float *mean_delta, float *variance_delta, int batch, int filters,
        int spatial, float *delta, cudaStream_t st) {
...
__global__ void variance_delta_kernel(float *x, float *delta, float *mean,
        float *variance, int batch, int filters, int spatial,
    ...
    variance_delta[i] *= -.5f
            * powf(variance[i] + .00001f, (float)(-3.f / 2.f));
}
...
__global__ void fast_mean_delta_kernel(float *delta, float *variance,
        int batch, int filters, int spatial, float *mean_delta) {
...
__global__ void fast_variance_delta_kernel(float *x, float *delta,
        float *mean, float *variance, int batch, int filters, int spatial,
        float *variance_delta) {
    const int threads = BLOCK;
    __shared__ float local[threads];
    ...
    if (id == 0) {
        ...
        variance_delta[filter] *= -.5f
                * powf(variance[filter] + .00001f, (float)(-3.f / 2.f));
    }
}

__global__ void mean_delta_kernel(float *delta, float *variance, int batch,
...
extern "C" void mean_delta_gpu(float *delta, float *variance, int batch,
        int filters, int spatial, float *mean_delta, cudaStream_t st) {
...
extern "C" void fast_variance_delta_gpu(float *x, float *delta, float *mean,
        float *variance, int batch, int filters, int spatial,
        float *variance_delta, cudaStream_t st) {
    fast_variance_delta_kernel<<<filters, BLOCK, 0, st>>>(x, delta, mean,
            variance, batch, filters, spatial, variance_delta);
    check_error(cudaPeekAtLastError());
}

__global__ void mean_kernel(float *x, int batch, int filters, int spatial,
        float *mean) {
...
__global__ void variance_kernel(float *x, float *mean, int batch, int filters, int spatial, float *variance) {
    float scale = 1.0 / float(batch * spatial - 1);
    int j, k;
    int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    ...
    for (j = 0; j < batch; ++j) {
        for (k = 0; k < spatial; ++k) {
            int index = j * filters * spatial + i * spatial + k;
            //variance[i] += powf((x[index] - mean[i]), 2.0f);
            variance_sum += powf((x[index] - mean[i]), 2.0f);
        }
    }
    variance_sum *= scale;
    //variance[i] *= scale;
    variance[i] = variance_sum;
}


__global__ void reorg_kernel(int N, float *x, int w, int h, int c, int batch,
...
__global__ void axpy_kernel(int N, float ALPHA, float *X, int OFFX, int INCX,
...
__global__ void pow_kernel(int N, float ALPHA, float *X, int INCX, float *Y,
        int INCY) {
    int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i < N)
...
extern "C" void normalize_gpu(float *x, float *mean, float *variance,
...
__global__ void l2norm_kernel(int N, float *x, float *dx, int batch,
...
extern "C" void l2normalize_gpu(float *x, float *dx, int batch, int filters,
...
__global__ void fast_variance_kernel(float *x, float *mean, int batch,
        int filters, int spatial, float *variance) {
    const int threads = BLOCK;
    __shared__ float local[threads];
...
extern "C" void fast_variance_gpu(float *x, float *mean, int batch,
        int filters, int spatial, float *variance, cudaStream_t st) {
    fast_variance_kernel<<<filters, BLOCK, 0, st>>>(x, mean, batch, filters,
            spatial, variance);
    check_error(cudaPeekAtLastError());
}

extern "C" void mean_gpu(float *x, int batch, int filters, int spatial,
        float *mean, cudaStream_t st) {
...
extern "C" void variance_gpu(float *x, float *mean, int batch, int filters,
        int spatial, float *variance, cudaStream_t st) {
    variance_kernel<<<cuda_gridsize(filters), BLOCK, 0, st>>>(x, mean, batch,
            filters, spatial, variance);
    check_error(cudaPeekAtLastError());
}

extern "C" void axpy_gpu(int N, float ALPHA, float * X, int INCX, float * Y,
        int INCY, cudaStream_t st) {
    axpy_gpu_offset(N, ALPHA, X, 0, INCX, Y, 0, INCY, st);
}

extern "C" void pow_gpu(int N, float ALPHA, float * X, int INCX, float * Y,
        int INCY, cudaStream_t st) {
    pow_kernel<<<cuda_gridsize(N), BLOCK, 0, st>>>(N, ALPHA, X, INCX, Y, INCY);
    check_error(cudaPeekAtLastError());
}

extern "C" void axpy_gpu_offset(int N, float ALPHA, float * X, int OFFX,
        int INCX, float * Y, int OFFY, int INCY, cudaStream_t st) {
...
extern "C" void copy_gpu_offset(int N, float * X, int OFFX, int INCX,
        float * Y, int OFFY, int INCY, cudaStream_t st) {
    copy_kernel<<<cuda_gridsize(N), BLOCK, 0, st>>>(N, X, OFFX, INCX, Y, OFFY,
            INCY);
    check_error(cudaPeekAtLastError());
}

__global__ void flatten_kernel(int N, float *x, int spatial, int layers,
...
extern "C" void flatten_gpu(float *x, int spatial, int layers, int batch,
        int forward, float *out, cudaStream_t st) {
...
extern "C" void reorg_gpu(float *x, int w, int h, int c, int batch, int stride,
        int forward, float *out, cudaStream_t st) {
...
extern "C" void mask_gpu(int N, float * X, float mask_num, float * mask,
        float val, cudaStream_t st) {
    mask_kernel<<<cuda_gridsize(N), BLOCK, 0, st>>>(N, X, mask_num, mask, val);
    check_error(cudaPeekAtLastError());
}

__global__ void scale_mask_kernel(int n, float *x, float mask_num,
...
extern "C" void scale_mask_gpu(int N, float * X, float mask_num,
        float * mask, float scale, cudaStream_t st) {
...
extern "C" void shortcut_gpu(int batch, int w1, int h1, int c1, float *add,
        int w2, int h2, int c2, float s1, float s2, float *out,
        cudaStream_t st) {
...
__global__ void smooth_l1_kernel(int n, float *pred, float *truth,
...
extern "C" void smooth_l1_gpu(int n, float *pred, float *truth, float *delta,
        float *error, cudaStream_t st) {
    smooth_l1_kernel<<<cuda_gridsize(n), BLOCK, 0, st>>>(n, pred, truth, delta,
            error);
    check_error(cudaPeekAtLastError());
}

__global__ void softmax_x_ent_kernel(int n, float *pred, float *truth,
        float *delta, float *error) {
    int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i < n) {
        float t = truth[i];
...
extern "C" void softmax_x_ent_gpu(int n, float *pred, float *truth,
        float *delta, float *error, cudaStream_t st) {
    softmax_x_ent_kernel<<<cuda_gridsize(n), BLOCK, 0, st>>>(n, pred, truth,
            delta, error);
    check_error(cudaPeekAtLastError());
}

__global__ void logistic_x_ent_kernel(int n, float *pred, float *truth,
        float *delta, float *error) {
    int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i < n) {
        float t = truth[i];
...
extern "C" void logistic_x_ent_gpu(int n, float *pred, float *truth,
        float *delta, float *error, cudaStream_t st) {
...
__global__ void l2_kernel(int n, float *pred, float *truth, float *delta,
        float *error) {
    int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i < n) {
        float diff = truth[i] - pred[i];
...
extern "C" void l2_gpu(int n, float *pred, float *truth, float *delta,
        float *error, cudaStream_t st) {
...
extern "C" void l1_gpu(int n, float *pred, float *truth, float *delta,
        float *error, cudaStream_t st) {
    l1_kernel<<<cuda_gridsize(n), BLOCK, 0, st>>>(n, pred, truth, delta, error);
    check_error(cudaPeekAtLastError());
}

__global__ void wgan_kernel(int n, float *pred, float *truth, float *delta,
        float *error) {
    int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i < n) {
...
extern "C" void wgan_gpu(int n, float *pred, float *truth, float *delta,
        float *error, cudaStream_t st) {
...
__global__ void weighted_sum_kernel(int n, float *a, float *b, float *s,
        float *c) {
    int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i < n) {
...
__global__ void deinter_kernel(int NX, float *X, int NY, float *Y, int B,
        float *OUT) {
    int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i < (NX + NY) * B) {
        int b = i / (NX + NY);
...
extern "C" void deinter_gpu(int NX, float *X, int NY, float *Y, int B,
...
__global__ void inter_kernel(int NX, float *X, int NY, float *Y, int B,
        float *OUT) {
    int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i < (NX + NY) * B) {
        int b = i / (NX + NY);
...
extern "C" void inter_gpu(int NX, float *X, int NY, float *Y, int B,
        float *OUT, cudaStream_t st) {
...
extern "C" void weighted_sum_gpu(float *a, float *b, float *s, int num,
        float *c, cudaStream_t st) {
    weighted_sum_kernel<<<cuda_gridsize(num), BLOCK, 0, st>>>(num, a, b, s, c);
    check_error(cudaPeekAtLastError());
}

__global__ void weighted_delta_kernel(int n, float *a, float *b, float *s,
        float *da, float *db, float *ds, float *dc) {
    int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i < n) {
        if (da)
            da[i] += dc[i] * s[i];
...
extern "C" void weighted_delta_gpu(float *a, float *b, float *s, float *da,
        float *db, float *ds, int num, float *dc, cudaStream_t st) {
...
__global__ void softmax_tree_kernel(float *input, int spatial, int batch,
        int stride, float temp, float *output, int groups, int *group_size,
        int *group_offset) {
    int id = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
...
extern "C" void softmax_tree(float *input, int spatial, int batch, int stride,
        float temp, float *output, tree hier, cudaStream_t st) {
...
__global__ void softmax_kernel(float *input, int n, int batch,
        int batch_offset, int groups, int group_offset, int stride, float temp,
        float *output) {
    int id = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (id >= batch * groups)
        return;
...
extern "C" void softmax_gpu(float *input, int n, int batch, int batch_offset,
...
__global__ void upsample_kernel(size_t N, float *x, int w, int h, int c,
...
extern "C" void upsample_gpu(float *in, int w, int h, int c, int batch,
        int stride, int forward, float scale, float *out, cudaStream_t st) {
