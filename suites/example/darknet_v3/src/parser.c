...
local_layer parse_local(list *options, size_params params) {
	...
	local_layer layer = make_local_layer(batch, h, w, c, n, size, stride, pad,
			activation);
...
layer parse_deconvolutional(list *options, size_params params) {
	...
	layer l = make_deconvolutional_layer(batch, h, w, c, n, size, stride,
			padding, activation, batch_normalize, params.net->adam);
...
convolutional_layer parse_convolutional(list *options, size_params params) {
	...
	convolutional_layer layer = make_convolutional_layer(batch, h, w, c, n,
			groups, size, stride, padding, activation, batch_normalize, binary,
			xnor, params.net->adam);
...
layer parse_crnn(list *options, size_params params) {
	...
	layer l = make_crnn_layer(params.batch, params.w, params.h, params.c,
			hidden_filters, output_filters, params.time_steps, activation,
			batch_normalize);
...
layer parse_rnn(list *options, size_params params) {
	...
	layer l = make_rnn_layer(params.batch, params.inputs, output,
			params.time_steps, activation, batch_normalize, params.net->adam);
...
layer parse_gru(list *options, size_params params) {
	...
	layer l = make_gru_layer(params.batch, params.inputs, output,
			params.time_steps, batch_normalize, params.net->adam);
...
layer parse_lstm(list *options, size_params params) {
	...
	layer l = make_lstm_layer(params.batch, params.inputs, output,
			params.time_steps, batch_normalize, params.net->adam);
...
layer parse_connected(list *options, size_params params) {
	...
	layer l = make_connected_layer(params.batch, params.inputs, output,
...
layer parse_yolo(list *options, size_params params) {
	...
	layer l = make_yolo_layer(params.batch, params.w, params.h, num, total,
			mask, classes);
...
layer parse_region(list *options, size_params params) {
	...
	layer l = make_region_layer(params.batch, params.w, params.h, num, classes,
			coords);
	assert(l.outputs == params.inputs);
...
detection_layer parse_detection(list *options, size_params params) {
	...
	detection_layer layer = make_detection_layer(params.batch, params.inputs,
...
crop_layer parse_crop(list *options, size_params params) {
	...
	crop_layer l = make_crop_layer(batch, h, w, c, crop_height, crop_width,
...
layer parse_reorg(list *options, size_params params) {
	...
	layer layer = make_reorg_layer(batch, w, h, c, stride, reverse, flatten,
			extra);
	return layer;
}
...
dropout_layer parse_dropout(list *options, size_params params) {
	...
	dropout_layer layer = make_dropout_layer(params.batch, params.inputs,
			probability);
...
layer parse_normalization(list *options, size_params params) {
	...
	layer l = make_normalization_layer(params.batch, params.w, params.h,
			params.c, size, alpha, beta, kappa);
	return l;
}
...
layer parse_upsample(list *options, size_params params, network *net) {
	...
	layer l = make_upsample_layer(params.batch, params.w, params.h, params.c,
			stride);
...
void parse_net_options(list *options, network *net) {
	...
	net->inputs = option_find_int_quiet(options, "inputs",
			net->h * net->w * net->c);
	net->max_crop = option_find_int_quiet(options, "max_crop", net->w * 2);
	net->min_crop = option_find_int_quiet(options, "min_crop", net->w);
	net->max_ratio = option_find_float_quiet(options, "max_ratio",
			(float) net->max_crop / net->w);
	net->min_ratio = option_find_float_quiet(options, "min_ratio",
			(float) net->min_crop / net->w);
...
network *parse_network_cfg(char *filename) {
	...
	while (n) {
		...
		l.learning_rate_scale = option_find_float_quiet(options,
				"learning_rate", 1);
...
void load_connected_weights(layer l, FILE *fp, int transpose) {
	fread_float_to_float(l.biases, sizeof(float), l.outputs, fp);
	fread_float_to_float(l.weights, sizeof(float), l.outputs * l.inputs, fp);
	if (transpose) {
	...
	if (l.batch_normalize && (!l.dontloadscales)) {
		fread_float_to_float(l.scales, sizeof(float), l.outputs, fp);
		fread_float_to_float(l.rolling_mean, sizeof(float), l.outputs, fp);
		fread_float_to_float(l.rolling_variance, sizeof(float), l.outputs,
				fp);
...
void load_batchnorm_weights(layer l, FILE *fp) {
	fread_float_to_float(l.scales, sizeof(float), l.c, fp);
	fread_float_to_float(l.rolling_mean, sizeof(float), l.c, fp);
	fread_float_to_float(l.rolling_variance, sizeof(float), l.c, fp);
#ifdef GPU
	if(gpu_index >= 0) {
...
void load_convolutional_weights_binary(layer l, FILE *fp) {
	fread_float_to_float(l.biases, sizeof(float), l.n, fp);
	if (l.batch_normalize && (!l.dontloadscales)) {
		fread_float_to_float(l.scales, sizeof(float), l.n, fp);
		fread_float_to_float(l.rolling_mean, sizeof(float), l.n, fp);
		fread_float_to_float(l.rolling_variance, sizeof(float), l.n, fp);
	}
	int size = l.c * l.size * l.size;
	int i, j, k;
	for (i = 0; i < l.n; ++i) {
		float mean = 0;
		fread_float_to_float(&mean, sizeof(float), 1, fp);
		for (j = 0; j < size / 8; ++j) {
			int index = i * size + j * 8;
			unsigned char c = 0;
...
void load_convolutional_weights(layer l, FILE *fp) {
	...
	fread_float_to_float(l.weights, sizeof(float), num, fp);
...
void load_weights_upto(network *net, char *filename, int start, int cutoff) {
	...
	for (i = start; i < net->n && i < cutoff; ++i) {
		...
		if (l.type == LOCAL) {
			...
			int size = l.size * l.size * l.c * l.n * locations;
			fread_float_to_float(l.biases, sizeof(float), l.outputs, fp);
			fread_float_to_float(l.weights, sizeof(float), size, fp);
#ifdef GPU
			if(gpu_index >= 0) {
				push_local_layer(l);
			}
