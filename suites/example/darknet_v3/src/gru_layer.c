...
layer make_gru_layer(int batch, int inputs, int outputs, int steps,
	...
	fprintf(stderr, "\t\t");
	*(l.uz) = make_connected_layer(batch * steps, inputs, outputs, LINEAR,
			batch_normalize, adam);
	l.uz->batch = batch;

	l.wz = malloc(sizeof(layer));
	fprintf(stderr, "\t\t");
	*(l.wz) = make_connected_layer(batch * steps, outputs, outputs, LINEAR,
			batch_normalize, adam);
	l.wz->batch = batch;

	l.ur = malloc(sizeof(layer));
	fprintf(stderr, "\t\t");
	*(l.ur) = make_connected_layer(batch * steps, inputs, outputs, LINEAR,
			batch_normalize, adam);
	l.ur->batch = batch;

	l.wr = malloc(sizeof(layer));
	fprintf(stderr, "\t\t");
	*(l.wr) = make_connected_layer(batch * steps, outputs, outputs, LINEAR,
			batch_normalize, adam);
	l.wr->batch = batch;

	l.uh = malloc(sizeof(layer));
	fprintf(stderr, "\t\t");
	*(l.uh) = make_connected_layer(batch * steps, inputs, outputs, LINEAR,
			batch_normalize, adam);
	l.uh->batch = batch;

	l.wh = malloc(sizeof(layer));
	fprintf(stderr, "\t\t");
	*(l.wh) = make_connected_layer(batch * steps, outputs, outputs, LINEAR,
			batch_normalize, adam);
	l.wh->batch = batch;
...
void forward_gru_layer(layer l, network net) {
	...
	for (i = 0; i < l.steps; ++i) {
		...
		weighted_sum_cpu(l.state, l.h_cpu, l.z_cpu, l.outputs * l.batch,
				l.output);
...
void update_gru_layer_gpu(layer l, update_args a, cudaStream_t st) {
	update_connected_layer_gpu(*(l.ur), a, st);
	update_connected_layer_gpu(*(l.uz), a, st);
	update_connected_layer_gpu(*(l.uh), a, st);
	update_connected_layer_gpu(*(l.wr), a, st);
	update_connected_layer_gpu(*(l.wz), a, st);
	update_connected_layer_gpu(*(l.wh), a, st);
}
...
void forward_gru_layer_gpu(layer l, network net) {
	...
	fill_gpu(l.outputs * l.batch * l.steps, 0, uz.delta_gpu, 1, net.st);
	fill_gpu(l.outputs * l.batch * l.steps, 0, ur.delta_gpu, 1, net.st);
	fill_gpu(l.outputs * l.batch * l.steps, 0, uh.delta_gpu, 1, net.st);

	fill_gpu(l.outputs * l.batch * l.steps, 0, wz.delta_gpu, 1, net.st);
	fill_gpu(l.outputs * l.batch * l.steps, 0, wr.delta_gpu, 1, net.st);
	fill_gpu(l.outputs * l.batch * l.steps, 0, wh.delta_gpu, 1, net.st);
	if (net.train) {
		fill_gpu(l.outputs * l.batch * l.steps, 0, l.delta_gpu, 1, net.st);
		copy_gpu(l.outputs * l.batch, l.state_gpu, 1, l.prev_state_gpu, 1, net.st);
	}
	...
	for (i = 0; i < l.steps; ++i) {
		...
		copy_gpu(l.outputs * l.batch, uz.output_gpu, 1, l.z_gpu, 1, net.st);
		axpy_gpu(l.outputs * l.batch, 1, wz.output_gpu, 1, l.z_gpu, 1, net.st);

		copy_gpu(l.outputs * l.batch, ur.output_gpu, 1, l.r_gpu, 1, net.st);
		axpy_gpu(l.outputs * l.batch, 1, wr.output_gpu, 1, l.r_gpu, 1, net.st);

		activate_array_gpu(l.z_gpu, l.outputs * l.batch, LOGISTIC, net.st);
		activate_array_gpu(l.r_gpu, l.outputs * l.batch, LOGISTIC, net.st);

		copy_gpu(l.outputs * l.batch, l.state_gpu, 1, l.forgot_state_gpu, 1, net.st);
		mul_gpu(l.outputs * l.batch, l.r_gpu, 1, l.forgot_state_gpu, 1, net.st);

		s.input_gpu = l.forgot_state_gpu;
		forward_connected_layer_gpu(wh, s);

		copy_gpu(l.outputs * l.batch, uh.output_gpu, 1, l.h_gpu, 1, net.st);
		axpy_gpu(l.outputs * l.batch, 1, wh.output_gpu, 1, l.h_gpu, 1, net.st);

		if (l.tanh) {
			activate_array_gpu(l.h_gpu, l.outputs * l.batch, TANH, net.st);
		} else {
			activate_array_gpu(l.h_gpu, l.outputs * l.batch, LOGISTIC, net.st);
		}

		weighted_sum_gpu(l.state_gpu, l.h_gpu, l.z_gpu, l.outputs * l.batch,
				l.output_gpu, net.st);
		copy_gpu(l.outputs * l.batch, l.output_gpu, 1, l.state_gpu, 1, net.st);
...
void backward_gru_layer_gpu(layer l, network net) {
	...
	for (i = l.steps - 1; i >= 0; --i) {
		if (i != 0)
			copy_gpu(l.outputs * l.batch, l.output_gpu - l.outputs * l.batch, 1,
					l.state_gpu, 1, net.st);
		else
			copy_gpu(l.outputs * l.batch, l.prev_state_gpu, 1, l.state_gpu, 1, net.st);
		float *prev_delta_gpu =
				(i == 0) ? 0 : l.delta_gpu - l.outputs * l.batch;
		...
		weighted_delta_gpu(l.state_gpu, l.h_gpu, l.z_gpu, prev_delta_gpu,
				uh.delta_gpu, uz.delta_gpu, l.outputs * l.batch, l.delta_gpu, net.st);

		if (l.tanh) {
			gradient_array_gpu(l.h_gpu, l.outputs * l.batch, TANH,
					uh.delta_gpu, net.st);
		} else {
			gradient_array_gpu(l.h_gpu, l.outputs * l.batch, LOGISTIC,
					uh.delta_gpu, net.st);
		}

		copy_gpu(l.outputs * l.batch, uh.delta_gpu, 1, wh.delta_gpu, 1, net.st);

		copy_gpu(l.outputs * l.batch, l.state_gpu, 1, l.forgot_state_gpu, 1, net.st);
		mul_gpu(l.outputs * l.batch, l.r_gpu, 1, l.forgot_state_gpu, 1, net.st);
		fill_gpu(l.outputs * l.batch, 0, l.forgot_delta_gpu, 1, net.st);
		...
		if (prev_delta_gpu)
			mult_add_into_gpu(l.outputs * l.batch, l.forgot_delta_gpu, l.r_gpu,
					prev_delta_gpu, net.st);
		mult_add_into_gpu(l.outputs * l.batch, l.forgot_delta_gpu, l.state_gpu,
				ur.delta_gpu, net.st);

		gradient_array_gpu(l.r_gpu, l.outputs * l.batch, LOGISTIC,
				ur.delta_gpu, net.st);
		copy_gpu(l.outputs * l.batch, ur.delta_gpu, 1, wr.delta_gpu, 1, net.st);

		gradient_array_gpu(l.z_gpu, l.outputs * l.batch, LOGISTIC,
				uz.delta_gpu, net.st);
		copy_gpu(l.outputs * l.batch, uz.delta_gpu, 1, wz.delta_gpu, 1, net.st);
	...
	copy_gpu(l.outputs * l.batch, end_state, 1, l.state_gpu, 1, net.st);
}
#endif
