...
void forward_route_layer_gpu(const route_layer l, network net)
{
	...
	for(i = 0; i < l.n; ++i) {
		...
		int input_size = l.input_sizes[i];
		for(j = 0; j < l.batch; ++j) {
			copy_gpu(input_size, input + j*input_size, 1, l.output_gpu + offset + j*l.outputs, 1, net.st);
		}
		offset += input_size;
	}
}
...
void backward_route_layer_gpu(const route_layer l, network net)
{
	...
	for(i = 0; i < l.n; ++i) {
		...
		int input_size = l.input_sizes[i];
		for(j = 0; j < l.batch; ++j) {
			axpy_gpu(input_size, 1, l.delta_gpu + offset + j*l.outputs, 1, delta + j*input_size, 1, net.st);
		}
		offset += input_size;
	}
}
