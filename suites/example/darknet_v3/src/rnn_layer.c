...
layer make_rnn_layer(int batch, int inputs, int outputs, int steps,
	...
	*(l.input_layer) = make_connected_layer(batch * steps, inputs, outputs,
			activation, batch_normalize, adam);
	l.input_layer->batch = batch;

	l.self_layer = malloc(sizeof(layer));
	fprintf(stderr, "\t\t");
	*(l.self_layer) = make_connected_layer(batch * steps, outputs, outputs,
			activation, batch_normalize, adam);
	l.self_layer->batch = batch;

	l.output_layer = malloc(sizeof(layer));
	fprintf(stderr, "\t\t");
	*(l.output_layer) = make_connected_layer(batch * steps, outputs, outputs,
			activation, batch_normalize, adam);
...
void backward_rnn_layer(layer l, network net) {
	...
	for (i = l.steps - 1; i >= 0; --i) {
		...
		copy_cpu(l.outputs * l.batch, self_layer.delta, 1, input_layer.delta,
				1);
		if (i > 0 && l.shortcut)
			axpy_cpu(l.outputs * l.batch, 1, self_layer.delta, 1,
					self_layer.delta - l.outputs * l.batch, 1);
...
void update_rnn_layer_gpu(layer l, update_args a, cudaStream_t st) {
	update_connected_layer_gpu(*(l.input_layer), a, st);
	update_connected_layer_gpu(*(l.self_layer), a, st);
	update_connected_layer_gpu(*(l.output_layer), a, st);
}
...
void forward_rnn_layer_gpu(layer l, network net) {
	...
	fill_gpu(l.outputs * l.batch * l.steps, 0, output_layer.delta_gpu, 1, net.st);
	fill_gpu(l.outputs * l.batch * l.steps, 0, self_layer.delta_gpu, 1, net.st);
	fill_gpu(l.outputs * l.batch * l.steps, 0, input_layer.delta_gpu, 1, net.st);
	...
	for (i = 0; i < l.steps; ++i) {
		...
		fill_gpu(l.outputs * l.batch, 0, l.state_gpu, 1, net.st);
		axpy_gpu(l.outputs * l.batch, 1, input_layer.output_gpu, 1, l.state_gpu,
				1, net.st);
		axpy_gpu(l.outputs * l.batch, 1, self_layer.output_gpu, 1, l.state_gpu,
				1, net.st);
...
void backward_rnn_layer_gpu(layer l, network net) {
	...
	for (i = l.steps - 1; i >= 0; --i) {
		...
		if (i != 0) {
			fill_gpu(l.outputs * l.batch, 0, l.state_gpu, 1, net.st);
			axpy_gpu(l.outputs * l.batch, 1,
					input_layer.output_gpu - l.outputs * l.batch, 1,
					l.state_gpu, 1, net.st);
			axpy_gpu(l.outputs * l.batch, 1,
					self_layer.output_gpu - l.outputs * l.batch, 1, l.state_gpu,
					1, net.st);
		} else {
			copy_gpu(l.outputs * l.batch, l.prev_state_gpu, 1, l.state_gpu, 1, net.st);
		}

		copy_gpu(l.outputs * l.batch, self_layer.delta_gpu, 1,
				input_layer.delta_gpu, 1, net.st);
		...
		if (net.delta_gpu)
			s.delta_gpu = net.delta_gpu + i * l.inputs * l.batch;
		else
			s.delta_gpu = 0;
	...
	fill_gpu(l.outputs * l.batch, 0, l.state_gpu, 1, net.st);
	axpy_gpu(l.outputs * l.batch, 1, last_input, 1, l.state_gpu, 1, net.st);
	axpy_gpu(l.outputs * l.batch, 1, last_self, 1, l.state_gpu, 1, net.st);
}
#endif
