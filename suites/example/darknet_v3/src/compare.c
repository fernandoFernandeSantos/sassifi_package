...
void train_compare(char *cfgfile, char *weightfile) {
	...
	while (1) {
		...
		if (i % 100 == 0) {
			char buff[256];
			sprintf(buff, "%s/%s_%d_minor_%d.weights", backup_directory, base,
					epoch, i);
...
void validate_compare(char *filename, char *weightfile) {
	...
	for (i = 1; i <= splits; ++i) {
		...
		printf("%d: Acc: %f, %lf seconds, %d images\n", i,
				(float) correct / total, sec(clock() - time), val.X.rows);
		free_data(val);
	}
}
...
int elo_comparator(const void*a, const void *b) {
	...
	if (box1.elos[current_class] == box2.elos[current_class])
		return 0;
	if (box1.elos[current_class] > box2.elos[current_class])
		return -1;
	return 1;
...
int bbox_comparator(const void *a, const void *b) {
	...
	memcpy(X + im1.w * im1.h * im1.c, im2.data,
			im2.w * im2.h * im2.c * sizeof(float));
...
void bbox_fight(network net, sortable_bbox *a, sortable_bbox *b, int classes,
		int class) {
...
void BattleRoyaleWithCheese(char *filename, char *weightfile) {
	...
	for (class = 0; class < classes; ++class) {
		...
		for (round = 1; round <= 100; ++round) {
			...
			for (i = 0; i < N / 2; ++i) {
				bbox_fight(net, boxes + i * 2, boxes + i * 2 + 1, classes,
						class);
			}
			qsort(boxes, N, sizeof(sortable_bbox), elo_comparator);
			if (round <= 20)
...
void run_compare(int argc, char **argv) {
	...
	else if (0 == strcmp(argv[2], "battle"))
