...
void delta_yolo_class(float *output, float *delta, int index, int class,
	...
	for (n = 0; n < classes; ++n) {
		delta[index + stride * n] = ((n == class) ? 1 : 0)
...
void forward_yolo_layer(const layer l, network net) {
	...
	*(l.cost) = 0;
	for (b = 0; b < l.batch; ++b) {
			...
			for (i = 0; i < l.w; ++i) {
				for (n = 0; n < l.n; ++n) {
					...
					if (best_iou > l.truth_thresh) {
						...
						int class = net.truth[best_t * (4 + 1) + b * l.truths
								+ 4];
						if (l.map)
							class = l.map[class];
		...
		for (t = 0; t < l.max_boxes; ++t) {
			box truth = float_to_box(net.truth + t * (4 + 1) + b * l.truths,
					1);
	...
	for (b = 0; b < l.batch; ++b) {
		for (n = 0; n < l.n; ++n) {
			...
			activate_array_gpu(l.output_gpu + index,
					(1 + l.classes) * l.w * l.h, LOGISTIC, net.st);
		}
	}
