...
void forward_softmax_layer(const softmax_layer l, network net) {
	if (l.softmax_tree) {
		...
		for (i = 0; i < l.softmax_tree->groups; ++i) {
			...
			softmax_cpu(net.input + count, group_size, l.batch, l.inputs, 1, 0,
					1, l.temperature, l.output + count);
			count += group_size;
		}
	} else {
	...
	if (net.truth && !l.noloss) {
		softmax_x_ent_cpu(l.batch * l.inputs, l.output, net.truth, l.delta,
				l.loss);
...
void forward_softmax_layer_gpu(const softmax_layer l, network net) {
	if (l.softmax_tree) {
		softmax_tree(net.input_gpu, 1, l.batch, l.inputs, l.temperature,
				l.output_gpu, *l.softmax_tree, net.st);
		/*
		 int i;
		 int count = 0;
		 for (i = 0; i < l.softmax_tree->groups; ++i) {
		...
		if (l.spatial) {
			softmax_gpu(net.input_gpu, l.c, l.batch * l.c, l.inputs / l.c,
					l.w * l.h, 1, l.w * l.h, 1, l.output_gpu, net.st);
		} else {
