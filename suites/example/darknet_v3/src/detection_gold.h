...
struct Detection {
	...
	Detection() :
			prob(std::vector<float>()), bbox(box()), nboxes(0), objectness(0), sort_class(
					0), classes(0) {
	}

	Detection(int classes, int nboxes, int sort_class, float objectness,
			std::vector<float> prob, box bb) :
	...
	Detection& operator=(const Detection& other) // copy assignment
		...
		if (this != &other) { // self-assignment check expected
			this->prob = std::vector < float > (other.prob);
...
struct DetectionGold {
	...
	DetectionGold(int argc, char **argv, float thresh, float hier_thresh,
	...
	int compare_line(float g_objectness, float f_objectness,
