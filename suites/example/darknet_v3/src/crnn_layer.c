...
layer make_crnn_layer(int batch, int h, int w, int c, int hidden_filters,
		int output_filters, int steps, ACTIVATION activation,
		int batch_normalize) {
	...
	fprintf(stderr, "\t\t");
	*(l.input_layer) = make_convolutional_layer(batch * steps, h, w, c,
			hidden_filters, 1, 3, 1, 1, activation, batch_normalize, 0, 0, 0);
	l.input_layer->batch = batch;

	l.self_layer = malloc(sizeof(layer));
	fprintf(stderr, "\t\t");
	*(l.self_layer) = make_convolutional_layer(batch * steps, h, w,
			hidden_filters, hidden_filters, 1, 3, 1, 1, activation,
			batch_normalize, 0, 0, 0);
	l.self_layer->batch = batch;

	l.output_layer = malloc(sizeof(layer));
	fprintf(stderr, "\t\t");
	*(l.output_layer) = make_convolutional_layer(batch * steps, h, w,
			hidden_filters, output_filters, 1, 3, 1, 1, activation,
			batch_normalize, 0, 0, 0);
...
void update_crnn_layer_gpu(layer l, update_args a, cudaStream_t st) {
	update_convolutional_layer_gpu(*(l.input_layer), a, st);
	update_convolutional_layer_gpu(*(l.self_layer), a, st);
	update_convolutional_layer_gpu(*(l.output_layer), a, st);
}
...
void forward_crnn_layer_gpu(layer l, network net) {
	...
	fill_gpu(l.hidden * l.batch * l.steps, 0, self_layer.delta_gpu, 1, net.st);
	fill_gpu(l.hidden * l.batch * l.steps, 0, input_layer.delta_gpu, 1, net.st);
	if (net.train)
		fill_gpu(l.hidden * l.batch, 0, l.state_gpu, 1, net.st);
	...
	for (i = 0; i < l.steps; ++i) {
		...
		if (l.shortcut) {
			copy_gpu(l.hidden * l.batch, old_state, 1, l.state_gpu, 1, net.st);
		} else {
			fill_gpu(l.hidden * l.batch, 0, l.state_gpu, 1, net.st);
		}
...
void backward_crnn_layer_gpu(layer l, network net) {
	...
	for (i = l.steps - 1; i >= 0; --i) {
		copy_gpu(l.hidden * l.batch, input_layer.output_gpu, 1, l.state_gpu, 1, net.st);
		axpy_gpu(l.hidden * l.batch, 1, self_layer.output_gpu, 1, l.state_gpu,
				1, net.st);
		...
		if (i > 0 && l.shortcut)
			axpy_gpu(l.hidden * l.batch, 1, self_layer.delta_gpu, 1,
					self_layer.delta_gpu - l.hidden * l.batch, 1, net.st);
