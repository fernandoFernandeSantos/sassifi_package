...
void forward_cost_layer(cost_layer l, network net) {
	...
	if (l.cost_type == SMOOTH) {
		smooth_l1_cpu(l.batch * l.inputs, net.input, net.truth, l.delta,
				l.output);
...
void forward_cost_layer_gpu(cost_layer l, network net) {
	if (!net.truth)
		return;
	if (l.smooth) {
		scal_gpu(l.batch * l.inputs, (1 - l.smooth), net.truth_gpu, 1, net.st);
		add_gpu(l.batch * l.inputs, l.smooth * 1. / l.inputs, net.truth_gpu, 1,
				net.st);
	}

	if (l.cost_type == SMOOTH) {
		smooth_l1_gpu(l.batch * l.inputs, net.input_gpu, net.truth_gpu,
				l.delta_gpu, l.output_gpu, net.st);
	} else if (l.cost_type == L1) {
		l1_gpu(l.batch * l.inputs, net.input_gpu, net.truth_gpu, l.delta_gpu,
				l.output_gpu, net.st);
	} else if (l.cost_type == WGAN) {
		wgan_gpu(l.batch * l.inputs, net.input_gpu, net.truth_gpu, l.delta_gpu,
				l.output_gpu, net.st);
	} else {
		l2_gpu(l.batch * l.inputs, net.input_gpu, net.truth_gpu, l.delta_gpu,
				l.output_gpu, net.st);
	}

	if (l.cost_type == SEG && l.noobject_scale != 1) {
		scale_mask_gpu(l.batch * l.inputs, l.delta_gpu, 0, net.truth_gpu,
				l.noobject_scale, net.st);
	...
	if (l.ratio) {
		...
		thresh = 0;
		printf("%f\n", thresh);
		supp_gpu(l.batch * l.inputs, thresh, l.delta_gpu, 1, net.st);
	}
...
void backward_cost_layer_gpu(const cost_layer l, network net) {
	axpy_gpu(l.batch * l.inputs, l.scale, l.delta_gpu, 1, net.delta_gpu, 1, net.st);
}
#endif
