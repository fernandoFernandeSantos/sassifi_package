...
void variance_delta_cpu(float *x, float *delta, float *mean,
	...
	for (i = 0; i < filters; ++i) {
		...
		variance_delta[i] *= -.5
				* pow(variance[i] + .00001f, (float)(-3. / 2.));
	}
}
		...
		for (f = 0; f < filters; ++f) {
			for (k = 0; k < spatial; ++k) {
				...
				delta[index] = delta[index] * 1. / (sqrt(variance[f] + .00001f))
						+ variance_delta[f] * 2. * (x[index] - mean[f])
								/ (spatial * batch)
...
void forward_batchnorm_layer_gpu(layer l, network net) {
	...
	copy_gpu(l.outputs * l.batch, l.output_gpu, 1, l.x_gpu, 1, net.st);
	if (net.train) {
#ifdef CUDNN
		float one = 1;
		float zero = 0;
		...
		fast_variance_gpu(l.output_gpu, l.mean_gpu, l.batch, l.out_c,
				l.out_h * l.out_w, l.variance_gpu, net.st);

		scal_gpu(l.out_c, .99, l.rolling_mean_gpu, 1, net.st);
		axpy_gpu(l.out_c, .01, l.mean_gpu, 1, l.rolling_mean_gpu, 1, net.st);
		scal_gpu(l.out_c, .99, l.rolling_variance_gpu, 1, net.st);
		...
		normalize_gpu(l.output_gpu, l.mean_gpu, l.variance_gpu, l.batch,
				l.out_c, l.out_h * l.out_w, net.st);
		copy_gpu(l.outputs * l.batch, l.output_gpu, 1, l.x_norm_gpu, 1, net.st);

		scale_bias_gpu(l.output_gpu, l.scales_gpu, l.batch, l.out_c,
				l.out_h * l.out_w, net.st);
		...
		normalize_gpu(l.output_gpu, l.rolling_mean_gpu, l.rolling_variance_gpu,
				l.batch, l.out_c, l.out_h * l.out_w, net.st);
		scale_bias_gpu(l.output_gpu, l.scales_gpu, l.batch, l.out_c,
				l.out_h * l.out_w, net.st);
		add_bias_gpu(l.output_gpu, l.biases_gpu, l.batch, l.out_c,
				l.out_w * l.out_h, net.st);
...
#endif
	if (l.type == BATCHNORM)
		copy_gpu(l.outputs * l.batch, l.delta_gpu, 1, net.delta_gpu, 1, net.st);
}
#endif
