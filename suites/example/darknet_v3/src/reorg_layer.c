...
void forward_reorg_layer_gpu(layer l, network net) {
	...
	if (l.flatten) {
		...
		for (i = 0; i < l.batch; ++i) {
			copy_gpu(l.inputs, net.input_gpu + i * l.inputs, 1,
					l.output_gpu + i * l.outputs, 1, net.st);
		}
	} else if (l.reverse) {
		reorg_gpu(net.input_gpu, l.w, l.h, l.c, l.batch, l.stride, 1,
				l.output_gpu, net.st);
	} else {
		reorg_gpu(net.input_gpu, l.w, l.h, l.c, l.batch, l.stride, 0,
				l.output_gpu, net.st);
	}
}
...
void backward_reorg_layer_gpu(layer l, network net) {
	if (l.flatten) {
		if (l.reverse) {
			flatten_gpu(l.delta_gpu, l.w * l.h, l.c, l.batch, 1, net.delta_gpu, net.st);
		} else {
			flatten_gpu(l.delta_gpu, l.w * l.h, l.c, l.batch, 0, net.delta_gpu, net.st);
		}
	} else if (l.extra) {
		int i;
		for (i = 0; i < l.batch; ++i) {
			copy_gpu(l.inputs, l.delta_gpu + i * l.outputs, 1,
					net.delta_gpu + i * l.inputs, 1, net.st);
		}
	} else if (l.reverse) {
		reorg_gpu(l.delta_gpu, l.w, l.h, l.c, l.batch, l.stride, 0,
				net.delta_gpu, net.st);
	} else {
		reorg_gpu(l.delta_gpu, l.w, l.h, l.c, l.batch, l.stride, 1,
				net.delta_gpu, net.st);
	}
}
#endif
