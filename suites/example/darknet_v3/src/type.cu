...
__global__ void cuda_f32_to_f16(float *X, size_t N, float_fp16 *Y) {
...
__global__ void cuda_f16_to_f32(float_fp16 *X, size_t N, float *Y) {
    size_t i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (i < N) {
...
void run_cuda_gemm_half(cublasHandle_t handle, int TA, int TB, int M, int N,
        int K, float ALPHA, float *A_gpu, int lda, float *B_gpu, int ldb,
        float BETA, float *C_gpu, int ldc, cudaStream_t st) {
   /* float_fp16 *a = nullptr;
    float_fp16 *b = nullptr;
    float_fp16 *c = nullptr;
...
//          a, M * K, b, K * N, c, M * N);
    check_error(cudaMalloc((void**) &a, sizeof(float_fp16) * siz_a));

    cuda_f32_to_f16<<<cuda_gridsize(siz_a), BLOCK, 0, st>>>(A_gpu, siz_a, a);
    check_error(cudaPeekAtLastError());

    check_error(cudaMalloc((void**) &b, sizeof(float_fp16) * siz_b));
    cuda_f32_to_f16<<<cuda_gridsize(siz_b), BLOCK, 0, st>>>(B_gpu, siz_b, b);
    check_error(cudaPeekAtLastError());

    if (BETA != 0) {
        check_error(cudaMalloc((void**) &c, sizeof(float_fp16) * siz_c));
...
#if __CUDA_ARCH__ > 600
    float_fp16 alpha = float_fp16(ALPHA);
    float_fp16 beta = float_fp16(BETA);
#ifndef OPENGEMM
