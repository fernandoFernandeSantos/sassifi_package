		...
		for (k = 0; k < c; ++k) {
			for (j = 0; j < h; ++j) {
				for (i = 0; i < w; ++i) {
					...
					int out_index = w2
							+ w * stride * (h2 + h * stride * (c2 + out_c * b));
					if (forward)
...
void shortcut_cpu(int batch, int w1, int h1, int c1, float *add, int w2,
	...
	for (b = 0; b < batch; ++b) {
		for (k = 0; k < minc; ++k) {
			for (j = 0; j < minh; ++j) {
				for (i = 0; i < minw; ++i) {
					int out_index = i * sample
							+ w2 * (j * sample + h2 * (k + c2 * b));
					int add_index = i * stride
							+ w1 * (j * stride + h1 * (k + c1 * b));
		...
		for (k = 0; k < c; ++k) {
			for (j = 0; j < h * stride; ++j) {
				for (i = 0; i < w * stride; ++i) {
					int in_index = b * w * h * c + k * w * h + (j / stride) * w
							+ i / stride;
