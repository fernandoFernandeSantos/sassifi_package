...
DetectionGold::DetectionGold(int argc, char **argv, float thresh,
		float hier_thresh, char *img_list_path, char *config_file,
...
bool operator!=(const box& a, const box& b) {
	float x_diff = std::fabs((a.x - b.x));
	float y_diff = std::fabs((a.y - b.y));
	float w_diff = std::fabs((a.w - b.w));
	float h_diff = std::fabs((a.h - b.h));

	return (x_diff > THRESHOLD_ERROR || y_diff > THRESHOLD_ERROR
			|| w_diff > THRESHOLD_ERROR || h_diff > THRESHOLD_ERROR);
}

bool operator!=(const std::tuple<float, float, float, float> f,
		std::tuple<float, float, float, float> g) {

	float x1_diff = std::fabs(std::get < 0 > (f) - std::get < 0 > (g));
	float x2_diff = std::fabs(std::get < 1 > (f) - std::get < 1 > (g));
	float x3_diff = std::fabs(std::get < 2 > (f) - std::get < 2 > (g));
	float x4_diff = std::fabs(std::get < 3 > (f) - std::get < 3 > (g));
...
int DetectionGold::compare_line(float g_objectness, float f_objectness,
		int g_sort_class, int f_sort_class, const box& g_box, const box& f_box,
		const std::string& img, int nb, int classes, const float* g_probs,
		const float* f_probs, int img_w, int img_h, int inet) {

	float objs_diff = std::fabs(g_objectness - f_objectness);
	...
	for (int cl = 0; cl < classes; ++cl) {
		float g_prob = g_probs[cl];
		float f_prob = f_probs[cl];
		float prob_diff = std::fabs(g_prob - f_prob);
...
int DetectionGold::cmp(detection* found_dets, int nboxes, int img_index,
	...
	for (int nb = 0; nb < nboxes; nb++) {
		...
		float g_objectness = g_det.objectness;
		float f_objectness = f_det.objectness;

		int g_sort_class = g_det.sort_class;
		int f_sort_class = f_det.sort_class;

		const float* g_probs = g_det.prob.data();
		const float* f_probs = f_det.prob;
...
Detection DetectionGold::load_gold_line(std::ifstream& gold_file, int nboxes) {
	...
	float objectness = std::stof(splited_line[0]);
...
//
//          // Getting the probabilities
//          std::vector < float > probs(classes, 0.0);
//
//          if (getline(gold_file, line)) {
//              splited_line = split(line, ';');
//
