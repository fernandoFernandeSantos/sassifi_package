...
__device__ float bilinear_interpolate_kernel(float *image, int w, int h,
...
__global__ void levels_image_kernel(float *image, float *rand, int batch,
        int w, int h, int train, float saturation, float exposure,
...
__global__ void forward_crop_layer_kernel(float *input, float *rand, int size,
        ...
        int flip, float angle, float *output) {
    int id = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (id >= size)
        return;
